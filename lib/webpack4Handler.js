...
const getNode = (block, type) => {
  ...
  switch (type) {
    ...
    case "ImportDependenciesBlock":
      ...
      chunks = block.chunkGroup?.chunks?.map (v => v.id)
      result = {
    ...
    case "NormalModule":
      ...
      for (let chunk of block._chunks) chunks.push (chunk.id)
...
module.exports = function (compilation) {
  ...
  const iteratorDep = (blocks, parentNode) => {
    blocks.forEach (block => {
      ...
      if (moduleSet.has (block)) {
        let currentNode = block._moduleNode
        OriginEdges.push ({
          source: parentNode.id,
          target: currentNode.id
        })
        currentNode.repeatNodes.push (id)
        moduleTree.parentNode = currentNode.id
      } else {
  ...
  const iteratorChunk = (chunk, block, parentNode) => {
    ...
    if (!moduleSet.has (block)) delete block._moduleNode
    ...
    if (moduleSet.has (block)) {
      ...
      if (modules.size) {
        ...
        interModules.forEach (b => iteratorChunk (chunk, b, moduleTree))
      }
      if (blocks.length) {
        blocks.forEach (b => {
          ...
          blockChunks.forEach (c => iteratorChunk (c, b, moduleTree))
        })
      }
    }
    parentNode.children.push (moduleTree)
  ...
  baseRoot.children = []
  iteratorDep (depEntries, baseRoot)
