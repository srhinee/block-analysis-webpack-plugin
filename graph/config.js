(function initG6 () {
	...
	const moduleMap = {
		root: { color: "#666" },
		ImportDependenciesBlock: { color: "rgba(238,86,86,0.66)" },
		AsyncDependenciesBlock: { color: "rgb(191,91,72)" },
		AMDRequireDependenciesBlock: { color: "rgba(238,160,160,0.66)" },
		NormalModule: { color: "#69c0ff" },
		ConcatenatedModule: { color: "#daa73e" },
		ContextModule: { color: "rgb(124,68,185)" },
		MultiModule: { color: "#35caa2" },
		CssModule: { color: "#7f4d37" }
	};
	const config = {
		container: "container", width, height, modes: {
			default: ["drag-canvas", "zoom-canvas", {
				type: "tooltip", formatText (item) {
		...
		nodeStateStyles: {
			hover: {
				lineWidth: 1, stroke: "#1890ff", fill: "#e6f7ff"
			}
		}
	};
	let isOriginal = true, isGraph = false;
	let graph;
	...
	function nodeHandler (node) {
		...
		node.labelCfg.style.fill = node.children?.length > 1 && node.collapsed ? "rgb(53,202,162)" : "#666";
		if (!node.originLabel) {
	...
	function initGraph (data) {
		...
		c.modes.default = c.modes.default.concat (
			[
				...
				performanceData.moduleNums < 500 ? "activate-relations" : ""
			]));
		c.defaultCombo = {
			type: "circle",
			style: {
	...
	function genTemplate (data) {
		...
		`;
		if (list.resource[1]) resourceTemplate += `
			<div class="cell">
		...
		switch (data.type) {
			case "ConcatenatedModule":
				let _dep = data.depList.filter (v => v.request).sort ((a, b) => {
			...
			case "ContextModule":
				...
				${data.depList.map ((dep) => `<div class="item">${dep}</div>`)
				.join ("")}
				</div>`;
				break;
			case "MultiModule":
	...
	button2.addEventListener ("click", function() {graph.fitView ();});
} ());
